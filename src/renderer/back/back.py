...
class Package:
  ...
  def get_protocol(protocol):
    return p_types.unassigned if not p_types.protocols.get(protocol) else p_types.protocols.get(protocol)
  def _ip_header(self):
    ip = {
...
def pkg_callback(win_pcap, param, header, pkt_data):
  time.sleep(5)
  p = Package(pkt_data)
  # print("TYPE: {}\nSRC: {}\t DST: {}\t\nLENGTH: {}\n".format(p.frame_type, parse_ip(p.saddr), parse_ip(p.daddr), p.tlen))
  print(p._ip_header())
  print(pkt_data)
